...
class IDOL(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #########################################
        # sam_checkpoint = "/share/home/liudun/paperguides/segment-anything/sam_vit_h_4b8939.pth"
        # device = "cuda:1"
        # model_type = "default"
        # import sys
        # sys.path.append("..")
        ...
        # sam = sam_model_registry["vit_h"](checkpoint="/share/home/liudun/paperguides/sam-hq/pretrained_checkpoint/sam_hq_vit_h.pth")
    ...
    def forward(self, batched_inputs):
        ...
        if self.training: 
            ...
            #在这里使用sam更换最后的mask结果
            # self.HQSAMmask_generation_before(video_output,batched_inputs[0]['file_names'])
            return video_output
        
    def HQSAMmask_generation_before(self,video_output,file_names_list):
        with multiprocessing.Pool() as pool:
            cv2results = pool.map(read_image_poolx, file_names_list)
        video_len=len(file_names_list)
        inst_count=len(video_output['pred_scores'])
        for inst_i in range(inst_count):
            for video_i in range(video_len):
                #分数小于平均值 （以后这里可以加一个阈值比如0.8以下的）
                if video_output['scores_all'][inst_i][video_i]<video_output['pred_scores'][inst_i] \
                    and video_output['pred_masks'][inst_i][video_i] is not None:
                    target_image=cv2results[video_i]
                    SAM_mask = self.HQSAMmask_points_generation(video_output['pred_masks'][inst_i][video_i],target_image)
                    if SAM_mask is not None:
                        video_output['pred_masks'][inst_i][video_i]=SAM_mask

    def replace_mask_to_samMaks(self,video_output_pred_masks,batched_inputs_files):
        video_file_names=[batched_inputs_files_item for batched_inputs_files_item in batched_inputs_files]
        with multiprocessing.Pool() as pool:
            cv2results = pool.map(read_image_poolx, video_file_names)
        for batched_inputs_files_index,batched_inputs_files_item in enumerate(batched_inputs_files):
            # image = cv2.imread(batched_inputs_files_item)
            # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            image=cv2results[batched_inputs_files_index]
            self.predictor.set_image(image)
            for indexx,itemm in enumerate(video_output_pred_masks): #遍历的是每个分类的数目
                for pred_masks_index,pred_masks_item in enumerate(itemm): #遍历的是视频中的所有帧
                    #pred_masks_item 表示 w*h的true/false mask
                    if batched_inputs_files_index==pred_masks_index:
                        # 将pred_masks_item 转成 boxes
                            # print(pred_masks_item)
                            if pred_masks_item==None:
                                continue
                            input_box=masks_to_boxes(pred_masks_item)
                            input_box=input_box.detach().numpy()
                            masks, _, _ = self.predictor.predict(
                                point_coords=None,
                                point_labels=None,
                                box=input_box[0][None, :],
                                multimask_output=False,
                            )
                            itemm[pred_masks_index]=torch.from_numpy(masks[0])
                            # itemm[pred_masks_index]=None
        return video_output_pred_masks
    
    def HQSAMmask_points_generation(self,target_mask,target_image):
        X = torch.nonzero(target_mask+0).float().cuda()
        if len(X)==0:
            return None
        centroids, labels = kmeans(X, n_clusters=8)
        input_point=centroids.cpu().numpy()
        ...
        return torch.from_numpy(masks[0])
    def HQSAMmask_box_generation(self,target_mask,target_image):
        self.predictor.set_image(target_image)
        input_box=masks_to_boxes(target_mask)
        input_box=input_box.detach().numpy()
        masks, _, _ = self.predictor.predict(
            point_coords=None,
            point_labels=None,
    ...
    def inference(self, outputs, tracker, ori_size, image_sizes):
        ...
        for inst_id,m in  enumerate(video_dict.keys()):
            ...
            logits_list_all.append(score_list_ori)
        ...
        if len(logits_list)>0:
            ...
            # 使用列表解析对每个内部列表进行 torch.stack，同时处理 None
            # temp_list=[item for sublist in logits_list_all for item in sublist if item is not None]
            # tensor_list = [torch.stack(sublist) if all(tensor is not None for tensor in sublist) else torch.zeros_like(temp_list[0]) for sublist in logits_list_all]
            for sublist in logits_list_all:
                for tensor_index,tensor_item in enumerate(sublist):
                    if tensor_item is None:
                        sublist[tensor_index]=torch.zeros(25) #只有ytvis数据集是40 / ovis数据集是25
            tensor_list = [torch.stack(sublist) for sublist in logits_list_all]
            # tensor_list = [torch.stack(sublist) if all(tensor is not None for tensor in sublist) else torch.zeros(40) for sublist in logits_list_all]
            # pred_cls_all=torch.cat(tensor_list, dim=0)
            pred_cls_all = torch.stack(tensor_list)
            ######################################################
        else:
            pred_cls = []
            ######################################################
        ...
        if len(pred_cls) > 0:
            if self.is_multi_cls:
                ...
                ######################################################
                # einston
                pred_cls_all = pred_cls_all.transpose(1, 2)
                # pred_cls_all=torch.unsqueeze(pred_cls_all, 0).view(1,pred_cls_all.shape[1],pred_cls_all.shape[0])
                scores_all = pred_cls_all[is_above_thres]
                ######################################################
        ...
        video_output = {
            ...
            "scores_all": scores_all.tolist() if len(out_scores)!=0 else [],
        }
...
def read_image_poolx(image_path):
    """读取图像的函数"""
...
def kmeans(data, n_clusters, max_iter=100):
    """
    优化后的 GPU 版本的 K-means 算法。
    """
    n_samples, n_features = data.shape
    device = data.device

    # 随机初始化质心
    centroids = data[torch.randperm(n_samples, device=device)[:n_clusters]].clone()
    ...
    for _ in range(max_iter):
        ...
        distances = torch.cdist(data, centroids)
        # 分配每个数据点到最近的质心
        _, new_labels = torch.min(distances, dim=1)

        # 仅当质心被分配到新的数据点时更新它们
        if (new_labels != labels).any():
            for i in range(n_clusters):
                if torch.sum(new_labels == i) > 0:
                    centroids[i] = torch.mean(data[new_labels == i], dim=0)
