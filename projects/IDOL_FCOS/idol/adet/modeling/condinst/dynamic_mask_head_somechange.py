...
class DynamicMaskHead(nn.Module):
    ...
    def __call__(self, mask_feats, mask_feat_stride, pred_instances, gt_instances=None):
        if self.training:
            ...
            gt_inds = pred_instances.gt_inds # 长度为99、110、126 。。。。
            ...
            if len(pred_instances) == 0:
                ...
                #mask_feats
                #mask_feat_stride = self.mask_branch.out_stride
                #pred_instances
                ...
                if self.boxinst_enabled:
                    ...
                    #gt_inds 实际上是为了重复image_color_similarity的某些行元素 
                    ##################################################################
                    #这个给 gt_inds 随机化处理：最大数字为instances长度 len(gt_inds)==len(gt_bitmasks)
                    ...
                    # torch.Size([102, 1, 320, 200])
                    # torch.Size([102, 1, 320, 200])
                    # torch.Size([62, 1, 272, 200])
                    # torch.Size([62, 1, 272, 200])
                    # exit(0)
                    ...
                    # print("self._iter.item()")
                    # print(self._iter.item())#1.0
                    # print("warmup_factor")
                    # print(warmup_factor)#0.0001
                    # exit(0)
                    loss_pairwise = loss_pairwise * warmup_factor
                    # print(loss_pairwise)#tensor(5.2197e-05, device='cuda:0', grad_fn=<MulBackward0>)
                    ...
                    # 相当于 dice_coefficient(inputs, targets)
