...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs,  targets, ref_target, indices, num_boxes):
        ...
        return losses
    def loss_no_masks(self, outputs,  targets, ref_target, indices, num_boxes,gt_instances):
        """Compute the losses related to the masks: the focal loss and the dice loss.
           targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        assert "pred_masks" in outputs
        ...
        #第一个损失函数  计算 pre_maskes 和gt_boxes 的关系
        loss_prj = compute_project_term(src_masks, target_masks)
        ...
        image_color_similarity = image_color_similarity[gt_inds].to(dtype=src_masks.dtype)
        self.pairwise_color_thresh=0.3
        weights = (image_color_similarity >= self.pairwise_color_thresh).float() * src_masks.float()
        ...
        # else:
        #     losses = {
        #     "loss_prj": (src_masks*0).sum(),
        #     "loss_pairwise": (src_masks*0).sum(),
        #     }
        return losses
    ################################################################################################################################################################
    def loss_boxinst(self, outputs,  targets, ref_target, indices, num_boxes, mask_logits, gt_instances=None):
        ...
        #手动构造 gt_inds
        # image_color_similarity的第一维是gt_inds中的最大数字‘   len(gt_instances)
        # mask_logits.shape(0)
        # src_masks = outputs["pred_masks"]  #list
        # if type(src_masks) == list:
        #     src_masks = torch.cat(src_masks, dim=1)[0]    
        num_boxes = mask_logits.shape[1]

        gt_inds=[]
        # if 'image_color_similarity' not in  gt_instances[0]:
        #     losses = {
        #         "loss_prj": (src_masks*0).sum(),
        #         "loss_pairwise": (src_masks*0).sum()
        #     }
