...
def instances_to_coco_json_video(inputs, outputs):
    ...
    ########################################################
    scores_all = outputs["scores_all"]
    ########################################################
    #####################################################
    # scores = outputs[0]['instances'].get("scores")
    # labels = outputs[0]['instances'].get("pred_classes")
    # masks = outputs[0]['instances'].get("pred_masks")

    # print(outputs[0]["pred_scores"])
    # print(labels)
    # print(masks)
    #####################################################
    ytvis_results = []
    ...
    for instance_id, (s, l, m, scores_all_item) in enumerate(zip(scores, labels, masks,scores_all)):
        segms = []
        for _mask in m:
            if _mask is not None:
                segms.append(mask_util.encode(np.array(_mask[:, :, None], order="F", dtype="uint8"))[0])
