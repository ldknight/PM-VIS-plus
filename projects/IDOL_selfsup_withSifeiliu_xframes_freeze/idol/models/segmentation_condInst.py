...
class CondInst_segm(nn.Module):
    def __init__(self, detr, rel_coord=True, freeze_detr=False):
        ...
        ###############################################################
        # selfsup. video_embed_head
        # self.video_embed_head = MLP(hidden_dim, hidden_dim, 128, 3)
        self.video_embed_head = MLP(hidden_dim, hidden_dim, hidden_dim, 3)
        ###############################################################
    ...
    ###############################################添加Boxinst系列参数
    # ,mask_feats=None  , pred_instances=None
    def forward(self, samples, det_targets, ref_targets, criterion, train=False, mask_feat_stride=8, gt_instances=None):
        ##################################################
        ...
        ##features  来自res2 --> res5四层输出结果
        srcs = []  #这里存 feature
        masks = [] #这里存 mask
        poses = [] #这里存 位置编码 
        spatial_shapes = [] #这里存 尺寸 

        for l, feat in enumerate(features[1:]):#这里只取后面三层res3--res5，l为序号、feat为mask+tensor
            # src: [N, _C, Hi, Wi],
            # mask: [N, Hi, Wi],
            # pos: [N, C, H_p, W_p]
        ...
        for lvl in range(enc_lay_num):
            ...
            ########################################################################################
            # 
            video_head_param = self.video_embed_head(hs[lvl])    # [bs, num_quries, num_params]
            video_head_params.append(video_head_param)

            video_head_param_ref = self.video_embed_head(hs_ref[lvl])    # [bs, num_quries, num_params]
            video_head_params_ref.append(video_head_param_ref)
            ########################################################################################
        ...
        ########################################################################
        #
        video_head_params_tensor = torch.stack(video_head_params)
        video_head_params_ref_tensor = torch.stack(video_head_params_ref)
        ########################################################################
        ...
        outputs['pred_logits'] = outputs_class[-1] #预测的类别
        outputs['pred_boxes'] = outputs_coord[-1]  #预测的boxes
        outputs['pred_masks'] = outputs_mask[-1]   #预测的mask
        outputs['pred_qd'] = contrast_items        #第二条分支 生成对抗的啥玩意
        ##################################################################
        #
        outputs['video_head_params_tensor'] = video_head_params_tensor[-1]
        outputs['video_head_params_ref_tensor'] = video_head_params_ref_tensor[-1]
        ##################################################################

        if self.detr.aux_loss:
            # outputs['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_mask)
            ######################################################################################################################################################
            outputs['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_mask,video_head_params_tensor,video_head_params_ref_tensor)
            ######################################################################################################################################################

        if train:
            # loss_dict = criterion(outputs, det_targets,ref_targets, indices_list)
            ############################################################
            # outputs 预测结果cls\box\mask
            # , mask_feats  , pred_instances 分别是 关键帧 和 参考帧
            # mask_feat_stride == 8
            loss_dict = criterion(outputs, det_targets, ref_targets, indices_list, mask_logits=self.mask_logits,mask_feat_stride=mask_feat_stride, gt_instances=gt_instances)
            ############################################################
        else:
            loss_dict = None
    ...
    # 共有
    # 在boxinst的别名叫 mask_heads_forward_with_coords
    ...
    ############################################################################################################################################
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_mask,video_head_params_tensor,video_head_params_ref_tensor):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_masks': c,'video_head_params_tensor':d,'video_head_params_ref_tensor':e}
                for a, b, c,d,e in zip(outputs_class[:-1], outputs_coord[:-1], outputs_mask[:-1],video_head_params_tensor[:-1],video_head_params_ref_tensor[:-1])]
    ############################################################################################################################################
...
'''boxInst的第 1 个损失函数 projection loss'''
# mask_scores 怎么求得？ mask_scores= mask_logits.sigmoid()
# gt_bitmasks 怎么求得？
def compute_project_term(mask_scores, gt_bitmasks):
    ...
    ########################################################
    # losses = {
    #     "loss_prj": (mask_losses_x + mask_losses_y).mean()
    #     }
    # return losses
    ########################################################

'''boxInst的第 2 个损失函数 pairwise loss'''
## mask_logits 怎么求得
def compute_pairwise_term(mask_logits, pairwise_size=3, pairwise_dilation=2):
    ...
    ########################################################
    # losses = {
    #     "loss_pairwise": -log_same_prob[:, 0]
    #     }
    # return losses
    ########################################################
################################################################################################################################################################

# 这个就是传说中的 controller，详情见condinst
class MLP(nn.Module):
