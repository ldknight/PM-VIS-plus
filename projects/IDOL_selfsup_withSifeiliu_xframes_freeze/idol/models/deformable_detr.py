...
class SetCriterion(nn.Module):
    ...
    def loss_selfsup_walk(self, outputs,  targets, ref_target, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, h, w), normalized by the image size.
        """
        assert 'video_head_params_tensor' in outputs
        assert 'video_head_params_ref_tensor' in outputs

        src_video_head_params = outputs['video_head_params_tensor']
        ref_video_head_params = outputs['video_head_params_ref_tensor']
        
        loss_walk=outputs['pred_boxes'].sum()*0
        not_zero_count=0
        for video_index,video in enumerate(self.batched_inputs):
            # # basepath="/share/home/liudun/VNext-main/projects/IDOL/video_embedding"
            # listdir = os.listdir(self.basepath)
            # target_video_id_info={}
            # if str(video["video_id"]) in listdir:
            #     # self.all_video_embedding_list[str(video["video_id"])]={}
            #     target_video_id_info=load_variavle(os.path.join(self.basepath,str(video["video_id"])))

            # # if str(video["video_id"]) not in self.all_video_embedding_list:
            # #     self.all_video_embedding_list[str(video["video_id"])]={}

            # frame_name=video["file_names"][0].split("/")[-1].split(".")[0]

            # # if str(frame_name) not in self.all_video_embedding_list[str(video["video_id"])]:
            # # if str(frame_name) not in target_video_id_info:

            # # self.all_video_embedding_list[str(video["video_id"])]=insert_item_to_json(self.all_video_embedding_list[str(video["video_id"])],str(frame_name),src_video_head_params[video_index].cpu())
            # # self.all_video_embedding_list[str(video["video_id"])]=insert_item_to_json(self.all_video_embedding_list[str(video["video_id"])],str(frame_name),src_video_head_params[video_index].cpu().detach().numpy().tolist())
            # # self.all_video_embedding_list[str(video["video_id"])][str(frame_name)]=src_video_head_params[video_index].cpu()

            # # self.all_video_embedding_list[str(video["video_id"])][str(frame_name)]=src_video_head_params[video_index].cpu().detach().numpy().tolist()
            # # self.all_video_embedding_list[str(video["video_id"])]=json.loads(json.dumps(self.all_video_embedding_list[str(video["video_id"])],sort_keys=True))

            # target_video_id_info[str(frame_name)]=src_video_head_params[video_index].cpu().detach().numpy().tolist()
            # target_video_id_info=json.loads(json.dumps(target_video_id_info,sort_keys=True))
            # save_variable(target_video_id_info,os.path.join(self.basepath,str(video["video_id"])))

            target_video_walk=[]
            for numx in range(video_index*int(src_video_head_params.shape[0]/len(self.batched_inputs)),(video_index+1)*int(src_video_head_params.shape[0]/len(self.batched_inputs))):
                  target_video_walk.append(src_video_head_params[numx])
                  target_video_walk.append(ref_video_head_params[numx])
            # target_video_walk=[src_video_head_params[video_index],ref_video_head_params[video_index]]

            
            # for frame_key in target_video_id_info:
            #     if int(frame_key)<=int(frame_name):
            #         target_video_walk.append(torch.tensor(target_video_id_info[frame_key]))
            # target_video_walk=target_video_walk[-2:]        
                #comput walk loss'
            target_video_walk_tensor=torch.stack(target_video_walk)
            target_video_walk_tensor=target_video_walk_tensor.transpose(0,1).transpose(0,2).unsqueeze(0)
            _,loss=self.CRW(target_video_walk_tensor)
            # print(loss)
            if loss[0]!=0:
                not_zero_count+=1
            loss_walk+=loss[0]
        ...
        #################################################################################
        # #重写 从key_frame & ref_frame 中合并对应的结果
        # 上面的结果只考虑了不连续性  并没有考虑现在所有的数据已经连续
        
        #################################################################################    
        if not_zero_count!=0:        
            loss_selfsup_walk=loss_walk/not_zero_count
        else:
            loss_selfsup_walk=loss_walk
        return {'loss_selfsup_walk':loss_selfsup_walk}   
    ...
    def get_loss(self, loss, outputs, targets, ref_target, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            ###############################################################################################
            'selfsup_walk': self.loss_selfsup_walk,
            ###############################################################################################
    ...
    ################################################################################################################################################
    # def get_loss_boxinst(self,loss,mask_logits, gt_instances=None, **kwargs):
    def get_loss_boxinst(self, loss, outputs, targets, ref_target, indices, num_boxes, gt_instances=None, **kwargs):
        loss_map = {
            'labels': self.loss_labels,
            'cardinality': self.loss_cardinality,
            'boxes': self.loss_boxes,
        ...
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        # return loss_map[loss](mask_logits, gt_instances, **kwargs)
        return loss_map[loss]( outputs, targets, ref_target, indices, num_boxes, gt_instances, **kwargs)
    ################################################################################################################################################
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    # if loss == "loss_boxinst":
                    if loss == "no_masks":
                        # l_dict = self.get_loss_boxinst(loss,mask_logits,gt_instances, **kwargs)
                        # l_dict = self.get_loss_boxinst(loss, outputs, targets, ref_target, indices, num_boxes, gt_instances, **kwargs)
                        # l_dict = self.loss_no_masks(aux_outputs, targets, ref_target, indices, num_boxes,gt_instances, **kwargs)
