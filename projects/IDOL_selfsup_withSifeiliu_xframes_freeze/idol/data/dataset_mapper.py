...
class YTVISDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            ...
            #######################################################
            ## 新添加 上面内容将被注释
            ref_frame = random.randrange(video_length-self.sampling_frame_num+1)
            selected_idx=[ref_frame+itemm for itemm in range(self.sampling_frame_num)]
            #######################################################
        else:
        ...
        for frame_idx in selected_idx:
            ...
            instances.remove("gt_masks")
            ################################################################################################################
            #过滤gt_masks 系列 用于boxinst
            # instances.pop("gt_masks", None)
            #         File "/share/home/liudun/paperguides/VNext/detectron2/structures/instances.py", line 65, in __getattr__
