...
class IDOL(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training: 
            ...
            ################################################################################################
            # if batched_inputs[0]['video_id']==1248:
            save_variable(video_output,"/share/home/liudun/paperguides/VNext/IDOL_swinL_ytvis19_inferenceTrain/"+str(batched_inputs[0]['video_id'])+".txt")
            # 使用IDOL_boxinst_inference替换gt_mask

            # target_pseudo_item = self.get_train_datasets(batched_inputs,video_output)
            # save_variable(target_pseudo_item,"/share/home/liudun/paperguides/VNext/pseudo_datasets_dict_RLE_IDOLBoxinstSwinLInferenceMask_gtBox_ytvis19_iii/"+str(batched_inputs[0]['video_id'])+".txt")
            
            # self.pseudo_datasets_dict.append(target_pseudo_item)
            # pseudo_datasets_dict.append(target_pseudo_item)

            # if self.gt_datasets_dict is not None:
                # if len(self.pseudo_datasets_dict)==len(self.gt_datasets_dict):
                # save_variable(self.pseudo_datasets_dict,"/share/home/liudun/paperguides/VNext/pseudo_datasets_dict_RLE_IDOLBoxinstSwinLInferenceMask_gtBox_ytvis19_ii.txt")
                # save_variable(pseudo_datasets_dict,"/share/home/liudun/paperguides/VNext/pseudo_datasets_dict_RLE_IDOLBoxinstSwinLInferenceMask_gtBox_ytvis19_ii.txt")
            ################################################################################################
            return video_output

    ####################################################################################
    # 处理单个视频
    def get_train_datasets(self,batched_inputs,video_output):
        if self.gt_datasets_dict is None:
            self.gt_datasets_dict = load_variavle("/share/home/liudun/VNext-main/datasets_dict_RLE_gt.txt")
        for datasets_dict_item in self.gt_datasets_dict:
            if datasets_dict_item['video_id']==batched_inputs[0]['video_id']:
                target_pseudo_item=copy.deepcopy(datasets_dict_item)
                target_pseudo_item['annotations']=[]
                ##############################################################################
                # # 首先判断总共有几个 类别
                # label_list=[]
                # for annotation_item in datasets_dict_item['annotations']:
                #     if len(annotation_item)!=0:
                #         label_list+=[category_item['category_id'] for category_item in annotation_item if category_item['category_id'] not in label_list]
                # # inference的结果 类别先对应上
                # inference_label_index = [index for index,item in enumerate(video_output['pred_labels']) if item in label_list]
                # if not len(inference_label_index): #有可能这个视频里就没有识别出目标
                #     print("视频里的目标没有识别出来")
                #     break
                ##############################################################################
                # tensor true/false mask生成box

                # # 先遍历对应的gt_box
                # for frame_gt_index,frame_gt_item in enumerate(datasets_dict_item['annotations']):
                #     #iou应该与类别绑定
                #     gtbox2inferenceboxIOU=[]
                #     for instance_gt in frame_gt_item:
                #         instance_gt['bbox']
                #         instance_gt['category_id']
                #         instancegt2inferenceboxiou=[-1 for i in video_output['pred_labels']]
                #         #遍历inference 
                #         for pred_label_index,pred_label_item in video_output['pred_labels']:
                #             #保证预测的label要与gt_label相同
                #             if pred_label_item==instance_gt['category_id']:
                #                 # 预测的mask不能是None
                #                 if video_output['pred_labels']['pred_masks'][pred_label_index][frame_gt_index] is None:
                #                     continue
                #                 # video_output['pred_labels']['pred_masks'][pred_label_index][frame_gt_index]   #pred_mask
                #                 inference_instance_box = masks_to_boxes(video_output['pred_labels']['pred_masks'][pred_label_index][frame_gt_index].unsqueeze(0))
                #                 # if img_mask_index>=len(target_pseudo_item['annotations']):
                #                 #     target_pseudo_item['annotations'].append([])
                #                 if int(inference_instance_box[0][0])>100000:
                #                     continue
                #                 gt_box = [instance_gt['bbox'][0],instance_gt['bbox'][1],instance_gt['bbox'][0]+instance_gt['bbox'][2],instance_gt['bbox'][1]+instance_gt['bbox'][3]]
                #                 IOu = generalized_box_iou(torch.tensor(np.array([gt_box])),inference_instance_box)
                #                 instancegt2inferenceboxiou[pred_label_index]=IOu[0]
                #         gtbox2inferenceboxIOU.append(instancegt2inferenceboxiou)

                #     all_pred_label_list=[]
                #     for itemmm in gtbox2inferenceboxIOU:
                #         pred_label_list=[]#某个pred_label 对应的与其他gtbox的iou
                #         for i in itemmm:
                #             pred_label_list.append(i)
                #         all_pred_label_list.append(pred_label_list)
                #     for index,item in enumerate(all_pred_label_list):
                #         #index 表示pred_label的下标
                #         i.max()的下标 就是gtbox对应的


                #     for instance_gt_index,instance_gt_item in enumerate(frame_gt_item):
                #         instance_gt_item['category_id']
                #         video_output['pred_labels'][]
                #     for indexx,itemm in enumerate(video_output['pred_labels']):
                #         aaa=[indexx]
                #         for indexx2,itemm2 in enumerate(video_output['pred_labels']):
                #             for indexx3,itemm3 in enumerate(video_output['pred_labels']):
                #             if itemm==itemm2:
                #     for gt_inference_box_IOU_list in gtbox2inferenceboxIOU:
                #         # 遍历video_output['pred_labels'] 中相同标签的下标

                #     for instance_gt_index,instance_gt_item in enumerate(frame_gt_item):
                #         for instance_gt_index2,instance_gt_item2 in enumerate(frame_gt_item):
                #             if instance_gt_index<instance_gt_index2:
                #                 instance_gt_item['category_id']==instance_gt_item2['category_id']


                for inference_label_index_i in range(0,len(video_output['pred_labels'])):
                    # 下面处理单个视频中的帧
                    for img_mask_index,img_mask_item in enumerate(video_output['pred_masks'][inference_label_index_i]):
                        # frame_info = []
                        if img_mask_item is not None:# 这一帧有instance标注信息
                            inference_img_instance_box = masks_to_boxes(img_mask_item.unsqueeze(0))

                            if img_mask_index>=len(target_pseudo_item['annotations']):
                                target_pseudo_item['annotations'].append([])

                            if int(inference_img_instance_box[0][0])>100000:
                                print("####int(inference_img_instance_box[0][0])>100000")
                                continue
                            max_iou=-1
                            max_iou_index=-1
                            for gt_img_instance_index,gt_img_instance in enumerate(datasets_dict_item['annotations'][img_mask_index]):
                                bbox_xywh = gt_img_instance['bbox']
                                gt_box = [bbox_xywh[0],bbox_xywh[1],bbox_xywh[0]+bbox_xywh[2],bbox_xywh[1]+bbox_xywh[3]]
                                IOu = generalized_box_iou(torch.tensor(np.array([gt_box])),inference_img_instance_box)
                                if IOu[0]>max_iou and IOu[0]>0 and gt_img_instance['category_id']==video_output['pred_labels'][inference_label_index_i]:
                                    max_iou=IOu[0]
                                    max_iou_index=gt_img_instance_index
                            # 替换掉 max_iou_index 位置的segment
                            # 使用gt_box
                            if max_iou_index==-1:
                                continue
                            target_instance = copy.deepcopy(datasets_dict_item['annotations'][img_mask_index][max_iou_index])
                            # encode(bimask) ==> RLE
                            target_instance['segmentation']=mask.encode(np.asfortranarray(img_mask_item.numpy()))
                            target_pseudo_item['annotations'][img_mask_index].append(target_instance)
                        else:
                            if img_mask_index>=len(target_pseudo_item['annotations']):
                                target_pseudo_item['annotations'].append([])
                ########################################################################
                # 合并box中符合要求的mask
                # temp_item_annotations=[]
                for index,item in enumerate(target_pseudo_item['annotations']):
                    # aa=[0 for x in range(0,len(item))]
                    for instance_index,instance_item in enumerate(item):
                        same_id_instance_list = []
                        for instance_index2,instance_item2 in enumerate(item):
                            if instance_index!=instance_index2 and instance_item['id']==instance_item2['id']:
                                same_id_instance_list.append(instance_item2)
                                # del item[instance_index2]
                                # aa[instance_index2]=1
                        instance_item['segmentation']=mask.decode(instance_item['segmentation'])+0
                        for same_id_instance_item in same_id_instance_list:
                            instance_item['segmentation']=mask.decode(same_id_instance_item['segmentation'])+0+instance_item['segmentation']
                        # # 框框之外的mask全部截去
                        instance_item['segmentation'][:,:int(instance_item['bbox'][0])]=0
                        instance_item['segmentation'][:int(instance_item['bbox'][1]),:]=0
                        instance_item['segmentation'][:,int(instance_item['bbox'][0]+instance_item['bbox'][2]):]=0
                        instance_item['segmentation'][int(instance_item['bbox'][1]+instance_item['bbox'][3]):,:]=0
                        instance_item['segmentation']=instance_item['segmentation']>0
                        #可视化
                        # save_mask_plt(batched_inputs[0]['file_names'][index],instance_item['segmentation'])
                        instance_item['segmentation']=mask.encode(instance_item['segmentation'])
                ########################################################################
                return target_pseudo_item
                # break#节省资源            
    ####################################################################################
...
def save_mask_plt(image_path,mask):
    ...
    plt.axis('off')
    plt.savefig('/share/home/liudun/paperguides/VNext/xxx1.jpg',bbox_inches='tight', pad_inches = -0.1) # 注意两个参数
    # plt.show() 
    plt.close()
################################################