...
class IDOL(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ##################################################################
            #
            images = self.preprocess_image2(batched_inputs)
            video_len =1
            ##################################################################
            ...
            ####################################################
            height = batched_inputs['height']
            width = batched_inputs['width']
            ####################################################
    ...
    def preprocess_image2(self, batched_inputs):
        ...
        """
        images = []
        images.append(self.normalizer(batched_inputs["image"].to(self.device)))
        images = ImageList.from_tensors(images)
        return images
