    ...
    for item in dataset_dicts: #遍历每个视频
        for img_index,img_item in enumerate(item["file_names"]):    #遍历每张图片
            ...
            with open(csv_path, encoding="utf-8-sig", mode="r") as f:
                ...
                for row in reader:
                    if temp_img_item in row["filename"]:#确定目标img
                        count_img_anno_flag+=1
                        # print(float(row["IOU"])<0.8)
                        if float(row["mask_IOU"])<0.5:
...
if __name__ == "__main__":
    dataset_dicts=load_variavle("/share/home/liudun/paperguides/VNext/dataset_dicts_RLE.txt")
    dataset_dicts=handle_dataset_dicts_sam(dataset_dicts)  
    save_variable(dataset_dicts,"/share/home/liudun/paperguides/VNext/ytvis19_dataset_dicts_sammask_samMask_maskiouOver05_RLE.txt")
