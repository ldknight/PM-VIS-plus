...
class IDOL(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #################################################################
        # from .util.predictor import VisualizationDemo
        ...
        self.save_visual_res_path = cfg.MODEL.IDOL.save_visual_res_path
        #################################################################
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ##########################################################################################
            #function for ytvis_video_demo
            self.matplot_predImages(batched_inputs,video_output)
            ##########################################################################################
            return video_output
        
    def matplot_predImages(self,batched_inputs,video_output):
        # filter_list = [index for index,item in enumerate(video_output['pred_scores']) if item>0] #只可视化分数大于0.5的
        # # filter_list = [index for index,item in enumerate(video_output['pred_scores']) if item>0.5] #只可视化分数大于0.5的
        # video_output['pred_scores']=[video_output['pred_scores'][item] for item in filter_list]
        # video_output['pred_labels']=[video_output['pred_labels'][item] for item in filter_list]
        # video_output['pred_masks']=[video_output['pred_masks'][item] for item in filter_list]

        # 示例使用
        image_paths = batched_inputs[0]['file_names']
        # labels = ["label1", "label2"]
        # labels = [item for index,item in enumerate(self.metadata.thing_classes) if index in video_output['pred_labels']]
        labels=[self.metadata.thing_classes[item] for item in video_output['pred_labels']]
        colors = [[np.random.rand() for _ in range(3)] for _ in labels]
        # mask_tensors = [torch.zeros((200, 200)), torch.zeros((200, 200))]  # 示例数据，需要您提供真实的mask数据

        save_base_path=os.path.join(self.save_visual_res_path,image_paths[0].split('/')[-2])
        os.makedirs(save_base_path, exist_ok=True)

        for iter,image_path in enumerate(image_paths):
            base_save_paths=os.path.join(save_base_path,image_path.split('/')[-1])
            # mask_tensors=[pred_masks_item[iter] for pred_masks_item in video_output['pred_masks']]

            for class_index,class_item in enumerate(labels):
                save_paths_list = base_save_paths.split('.')
                save_paths=save_paths_list[0]+"-"+str(class_index)+"."+save_paths_list[1]
                # save_paths=os.path.join(save_paths_list[0],"-",str(class_index),save_paths_list[1])
                self.draw_and_save_instances(image_path, [class_item], [video_output['pred_masks'][class_index][iter]], save_paths,[colors[class_index]])



    def draw_and_save_instances(self,image_path, labels, mask_tensors, save_path,colors):
        """
        绘制图像、边界框、实例分割掩码和标签，并保存到本地。
        
        参数:
        - image_path: 图像路径
        - labels: 图像的标签列表
        - mask_tensors: PyTorch张量格式的二进制掩码列表
        - save_path: 保存图像的路径
        """
        ...
        # 转换掩码为numpy数组并绘制
        for label, mask_tensor, color in zip(labels, mask_tensors, colors):
            ...
            mask_numpy = mask_tensor.cpu().numpy()
             ...
             # 获取边界框坐标
            bbox = self.get_bbox_from_mask(mask_tensor)
            if bbox:
                x_min, y_min, x_max, y_max = bbox
                rect = plt.Rectangle((x_min, y_min), x_max - x_min, y_max - y_min,
                                    fill=False, edgecolor=color, linewidth=2)
                ax.add_patch(rect)
                ax.text(x_min, y_min, label, fontsize=12, color='white', bbox=dict(facecolor=color, alpha=0.7))
            # 绘制分割掩码
            # 创建新的masked_image图层，与原始图像大小相同
            masked_image = np.copy(image)
            # masked_image = np.zeros_like(image)
            for c in range(3):
                channel = masked_image[:, :, c]
                channel[mask_numpy] = np.round(color[c] * 255).astype(np.uint8)
                channel[~mask_numpy] = image[~mask_numpy, c]  # 使用原始图像颜色值
            plt.imshow(masked_image, alpha=0.6)

            # 添加标签
            # x_min, y_min, x_max, y_max = 0, 0, image.shape[1], image.shape[0]
            # ax.text(x_min, y_min - 5, label, fontsize=12, color='white', bbox=dict(facecolor=color, alpha=0.7))
    ...
    def get_bbox_from_mask(self,mask_tensor):
        """
        从二进制的Tensor格式的mask中提取边界框。
        
        参数:
        - mask_tensor: PyTorch张量格式的二进制掩码（True/False）
        
        返回:
        - 边界框的坐标：(x_min, y_min, x_max, y_max)
        """
        # 将True/False转换为0/1
        ...
        if non_zero_indices.numel() == 0:  # 没有非零像素
            return None
    ...
    def ytvis_video_demo(self,batched_inputs,video_output):
        allimgs_mask_arr = []   #mask
        allimgs_scores_arr = [] #scores
        allimgs_labels_arr = [] #labels

        for img_indexx in range(0,len(video_output['pred_masks'][0])):
            allimgs_arr_item = []

            allimgs_scores_arr.append(video_output['pred_scores'])
            
            allimgs_labels_arr.append(video_output['pred_labels'])

            for class_item in video_output['pred_masks']:
                allimgs_arr_item.append(class_item[img_indexx])    
            allimgs_mask_arr.append(allimgs_arr_item)

        #trans for instance
        image_size = video_output['image_size']
        processed_results = []
        for img_index,img_item in enumerate(allimgs_mask_arr): #循环每一张图片
            result = Instances(image_size)
            # filter None item
            arr_index = []
            for indexx,aa in enumerate(img_item):
                if not aa is None:
                    arr_index.append(True)
                else:
                    arr_index.append(False)   
            target_img_item=[]
            temp_allimgs_scores_arr=[]
            temp_allimgs_labels_arr=[]
            for index,item in enumerate(arr_index):
                if item:
                    target_img_item.append(img_item[index])
                    temp_allimgs_scores_arr.append(allimgs_scores_arr[img_index][index])
                    temp_allimgs_labels_arr.append(allimgs_labels_arr[img_index][index])
            allimgs_scores_arr[img_index]=temp_allimgs_scores_arr
            allimgs_labels_arr[img_index]=temp_allimgs_labels_arr
            
            img_item_tensor = torch.tensor([aa.tolist() for aa in target_img_item])
            pred_boxes_from_mask = masks_to_boxes(img_item_tensor)

            img_item_tensor = img_item_tensor.unsqueeze(1)
            result.pred_masks = img_item_tensor
            # pred_boxes = [[0,0,0,0] for aa in target_img_item]
            # result.pred_boxes=Boxes(torch.tensor(pred_boxes))
            result.pred_boxes=Boxes(pred_boxes_from_mask)

            result.scores = torch.tensor(allimgs_scores_arr[img_index])
            result.pred_classes =torch.tensor(allimgs_labels_arr[img_index])
            if len(target_img_item)==0:
                r = result
            else:
                r = segmentation_postprocess(result, image_size[0], image_size[1])
            processed_results.append({"instances": r})


        self.handle_video_demo(batched_inputs,processed_results)

        # return processed_results


    def handle_video_demo(self,batched_inputs,processed_results):
        # video_input=""
        base_image_name = batched_inputs[0]['file_names'][0]
        base_image_name_arr=base_image_name.split('/')
        # output_fname=os.path.join("/share/home/liudun/VNext-main/demo/demo_res/ytvis_mp4/",base_image_name_arr[-2]+".mp4")
        output_fname=os.path.join(self.save_visual_res_path,base_image_name_arr[-2]+".mp4")
        width=batched_inputs[0]['width']
        height=batched_inputs[0]['height']
        num_frames=batched_inputs[0]['length']
        ...
        for vis_frame in tqdm.tqdm(self.demo.run_on_video(batched_inputs[0]['file_names'],processed_results), total=num_frames):
            # if args.output:
    ...
    def preprocess_coco_image(self, batched_inputs):
        ...
        #####################################################################
        # images = []
        # for itemx in batched_inputs:
        #     for itemx_tensor in itemx["image"]:
        #         images.append(self.normalizer(itemx_tensor.to(self.device)))
        #####################################################################
        images = ImageList.from_tensors(images)
        return images
...
#         self.instances_confidence_thresh = 0.5
...
#     def run_on_video(self, image_list,predictors):
#         """
...
#             if predictions.get('pred_masks').shape[0]==0:
#                 vis_frame = frame
#                 vis_frame = cv2.cvtColor(vis_frame, cv2.COLOR_RGB2BGR)
#             else:
#                 predictions=predictions[predictions.scores>self.instances_confidence_thresh]
...
#             img = read_image(os.path.join("/share/home/liudun/paperguides/VNext",frame), format="BGR")
#             yield process_predictions(img, predictors[frame_index])
