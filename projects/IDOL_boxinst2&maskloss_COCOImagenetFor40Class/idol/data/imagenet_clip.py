...
__all__ = ["Imagenet_CLIP_DatasetMapper"]
...
# ImageNet_TO_YTVIS_2019
class Imagenet_CLIP_DatasetMapper:
    """
    ...
    def __init__(self, cfg, is_train=True):
        if cfg.INPUT.CROP.ENABLED and is_train:
            ...
            self.crop_gen = None
        ############################################################
        # self.src_dataset_name="imagenet_train_fake"
        # self.tgt_dataset_name="ytvis_2019_train"
        # self.src_metadata = MetadataCatalog.get(self.src_dataset_name)
        # self.tgt_metadata = MetadataCatalog.get(self.tgt_dataset_name)
        # src2tgt = ImageNet_TO_YTVIS_2019
        # self.src2tgt = {}
    ...
    def __call__(self, dataset_dict):  # only used for training
        ...
        if not self.same_crop: #crop twice independently
            ...
            # ################################################################################
            # # einston
            # instances_key.gt_classes = torch.tensor(
            #         [self.src2tgt[c] if c in self.src2tgt else -1 for c in instances_key.gt_classes.tolist()]
            #     )
            # ################################################################################
            ...
            # ################################################################################
            # # einston
            # instances_ref.gt_classes = torch.tensor(
            #         [self.src2tgt[c] if c in self.src2tgt else -1 for c in instances_ref.gt_classes.tolist()]
            #     )
            # ################################################################################
            ...
            dataset_dict["instances"] = [filter_empty_instances(instances=instances_key,by_mask=False),  filter_empty_instances(instances=instances_ref,by_mask=False)]
