...
"""
    ...
    ########################################    
    # # filter the exist inference res in foler
    save_inference_path="/ourdisk/spaceB/liudun/project_code/VNext/inference_trainSet_Res/IDOL_allLoss_ImagenetCOCO_swinL_ytvis19_inferenceTrain"
    if not os.path.exists(save_inference_path):
        os.mkdir(save_inference_path)
    file_list_arr = os.listdir(save_inference_path)
    video_id_list = [int(item.split(".txt")[0]) for item in file_list_arr]
    dataset_dicts = [item for item in dataset_dicts if item['video_id'] not in video_id_list]
    ########################################    
