...
class IDOL(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ####################################################################################
            #在这里对true/false结果可视化
            # target_img = video_output['pred_masks'][0][0] #true/false tensor
            # target_img=target_img.type(torch.uint8)
            # target_img=target_img*255

            # img_numpy = target_img.numpy()
            # img = np.uint8(img_numpy)
            # import cv2
            # cv2.imwrite("/share/home/liudun/VNext-main/testtes2.png",img)
            ####################################################################################
            ####################################################################################
            ## 用于实验box mask
            from .util.box_ops import masks_to_boxes
            for pre_cls_item in video_output['pred_masks']:
                for pre_mask_index,pre_mask_item in enumerate(pre_cls_item):

                    # target_img = pre_cls_item[pre_mask_index] #true/false tensor
                    # target_img=target_img.type(torch.uint8)
                    # target_img=target_img*255
                    # img_numpy = target_img.numpy()
                    # img = np.uint8(img_numpy)
                    # import cv2
                    # cv2.imwrite("/share/home/liudun/VNext-main/testtes4.png",img)

                    #mask转box
                    # target_img = video_output['pred_masks'][0][0] #true/false tensor
                    target_img = pre_mask_item #true/false tensor
                    if target_img is not None:
                        boxes_res = masks_to_boxes(target_img.unsqueeze(0))
                        # boxes_res = box_cxcywh_to_xyxy(boxes_res)
                        # 将box位置转换为0/1
                        base_boxeslike_mask = torch.zeros(target_img.shape[0],target_img.shape[1])
                        base_boxeslike_mask = base_boxeslike_mask.numpy()
                        boxes_res_np = boxes_res.squeeze(0).numpy()
                        for indexx in range(int(boxes_res_np[0]),int(boxes_res_np[2])):#x
                            for indexx2 in range(int(boxes_res_np[1]),int(boxes_res_np[3])):#y
                                base_boxeslike_mask[indexx2][indexx]=1
                        target_base_boxeslike_mask = torch.tensor(base_boxeslike_mask,dtype=torch.bool)
                        pre_cls_item[pre_mask_index]=target_base_boxeslike_mask
                ...
                #     img = np.uint8(img_numpy)
                #     import cv2
                #     cv2.imwrite("/share/home/liudun/VNext-main/testtes5.png",img)
            #         break
            #     break
            # target_img = video_output['pred_masks'][0][0] #true/false tensor
            # target_img=target_img.type(torch.uint8)
            ...
            # img = np.uint8(img_numpy)
            # import cv2
            # cv2.imwrite("/share/home/liudun/VNext-main/testtes6.png",img)
            ####################################################################################
            return video_output
