...
class IDOL(nn.Module):
        ...
        if self.training:    
            ...
            for x in batched_inputs:
                ...
                for itemm in x["instances"]:
                    # item_index +=1
                    # if itemm.get('gt_ids').shape[0]==0:
                    #     temp_arr.append(item_index)
                    #     # if item_index%2==1:
                    #     #     temp_arr.append(int(item_index+1))
                    #     continue
                    gt_instances.append(itemm.to(self.device))
                # xitem_index = 0
                for xitem in x["image"]:
                    # xitem_index +=1
                    # if xitem_index in temp_arr:
                    #     continue
    ...
    def ytvis_video_demo(self,batched_inputs,video_output):
        ...
        processed_results = []
        for img_index,img_item in enumerate(allimgs_mask_arr):
    ...
    def handle_video_demo(self,batched_inputs,processed_results):
        ...
        output_fname=os.path.join("/share/home/liudun/paperguides/VNext/demo/demo_img_res/ytvis_2019_res/ytvis_mp4_boxinst_tracker/",base_image_name_arr[-2]+".mp4")
        width=batched_inputs[0]['width']
        height=batched_inputs[0]['height']
