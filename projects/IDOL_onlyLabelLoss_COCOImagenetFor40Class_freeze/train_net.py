...
def filter_imagenet(dataset_dict):
    ImageNet_TO_YTVIS_2019 = {#21个
        1:2, 2:3, 3:4, 4:7, 5:9, 6:10, 7:11, 8:12, 9:13, 10:16, 11:19, 12:23, 13:24, 14:25, 15:26, 16:31, 17:35, 18:36, 19:37, 20:38, 21:39     
    }
...
def main(args):
    ...
    # model = Trainer.build_model(cfg)
    # DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(cfg.MODEL.WEIGHTS, resume=args.resume)
    # print(Trainer.checkpointer)
    
    # return Trainer.train(cfg,model)
    trainer = Trainer(cfg)
    trainer.resume_or_load(resume=args.resume)
    layers_to_freeze = ["class_embed"]#,"query_embed","reference_points"

    # 冻结指定层的参数
    for param in trainer.checkpointer.model.parameters():
        param.requires_grad = False

    for name, parameter in trainer.checkpointer.model.named_parameters():
        if any(layer_name in name for layer_name in layers_to_freeze):
            parameter.requires_grad = True

    trainer._trainer.optimizer=trainer.build_optimizer(cfg, trainer.checkpointer.model)
    trainer.scheduler = trainer.build_lr_scheduler(cfg, trainer._trainer.optimizer)
    print("Frozen items....")
    for name, param in trainer.checkpointer.model.named_parameters():
        frozen_status=""
        if param.requires_grad:
            frozen_status = "not Frozen"
            print(f"{name}: {frozen_status}")
