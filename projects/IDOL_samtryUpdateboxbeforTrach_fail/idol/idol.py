...
class IDOL(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training: 
            ...
            #在这里使用sam更换最后的mask结果
            # video_output['pred_masks']=self.replace_mask_to_samMaks(video_output['pred_masks'],batched_inputs[0]['file_names'])

            # 测试一下看预测的box与mask生成的box有什么区别
            self.visible_predbox_maskbox(video_output['pred_boxes'],video_output['pred_masks'])
            return video_output
        
    def visible_predbox_maskbox(self,pred_boxes,pred_masks):
        for indexx,itemm in enumerate(pred_masks): #遍历的是每个分类的数目
            for pred_masks_index,pred_masks_item in enumerate(itemm): #遍历的是视频中的所有帧
                ...
                print(input_box,pred_boxes[indexx][pred_masks_index])
    ...
    def inference(self, outputs, tracker, ori_size, image_sizes):
        ...
        for inst_id,m in  enumerate(video_dict.keys()):
            ...
            for n in range(vid_len):
                ...
                if mask_i is None:    
                    ...
                    ######################################################
                    # ori_size[0]/ori_size[1]===> 720/1280
                    boxes_list_i.append(torch.tensor([float(box_i[0])*ori_size[1],float(box_i[1])*ori_size[0],float(box_i[2])*ori_size[1],float(box_i[3])*ori_size[0]]))
                    ######################################################
        ...
        if len(pred_cls) > 0:
            if self.is_multi_cls:#true
                is_above_thres = torch.where(pred_cls > self.apply_cls_thres)#self.apply_cls_thres==0.05
                scores = pred_cls[is_above_thres]
                labels = is_above_thres[1]
                out_masks = [masks_list[valid_id] for valid_id in is_above_thres[0]]
                ########################################################################
                out_boxes = [boxes_list[valid_id] for valid_id in is_above_thres[0]]
                ########################################################################
            else:
