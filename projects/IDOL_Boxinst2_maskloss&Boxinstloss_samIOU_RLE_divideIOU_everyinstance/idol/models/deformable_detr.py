...
# 用于condinst
def sigmoid_focal_loss_divideIOU(inputs, targets, num_boxes,target_ious, alpha: float = 0.25, gamma: float = 2):
    """
    ...
    if alpha >= 0:
        ...
        loss = alpha_t * loss
    ##############################    
    loss_mean_1=loss.mean(1)/target_ious # 对应目标除以IOU

    ##############################    
    # loss_mean_1_sum=loss.mean(1).sum()
    # return loss.mean(1).sum() / num_boxes

    return loss_mean_1.sum() / num_boxes

def dice_loss_divideIOU(inputs, targets, num_boxes,target_ious):
    """
    ...
    return (loss/target_ious).sum() / num_boxes
############################################################################################################
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs,  targets, ref_target, indices, num_boxes):
        ...
        ############################################################################
        key_frame_IOUs = [t["IOUs"] for t in targets]
        ############################################################################
        ...
        #############################################
        max_size=0
        for itemm in key_frame_IOUs:
            if itemm.size(0)>max_size:
                max_size=itemm.size(0)

        for indexx,itemm in enumerate(key_frame_IOUs):
           if itemm.size(0)<max_size:   
                # 将尺寸变为最大尺寸
                temp_itemm=itemm.tolist()
                for i in range(0,max_size):
                    if len(temp_itemm)<max_size:
                        temp_itemm.append(0)
                key_frame_IOUs[indexx]=torch.tensor(temp_itemm).to(itemm)

        key_frame_IOUs=torch.stack(key_frame_IOUs)
        tgt_iou_list = []
        #############################################
        for batch_idx in range(batch_size):
            ...
            ####################################################################################################
            batch_IOUs = key_frame_IOUs[batch_idx][:valid_num][gt_multi_idx]
            tgt_iou_list.append(batch_IOUs)
            ####################################################################################################
        ...
        if len(tgt_mask_list) != 0:
            ...
            #####################################################################
            target_ious = torch.cat(tgt_iou_list)
            #####################################################################
            ...
            # 33
            # exit(0)
            loss_mask_ = sigmoid_focal_loss_divideIOU(src_masks, target_masks, num_boxes,target_ious)
            loss_dice_ = dice_loss_divideIOU(src_masks, target_masks, num_boxes,target_ious)

            # temp_=torch.tensor([0.0]).to(loss_mask_.device)
            # count_=0
            # for itemm in targets:
            #     if itemm["IOUs"].shape[0]:
            #         temp_+=itemm["IOUs"].mean()
            #         count_+=1
            # if count_:
            #     temp_=temp_/count_
            #     losses = {
            #         "loss_mask": loss_mask_/temp_,
            #         "loss_dice": loss_dice_/temp_
            #     }
            # else:
            #     losses = {
